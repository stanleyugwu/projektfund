...
import { __PropertySchema, __UpdatePropertySchema } from "@/schema/property.schema"
...
export async function createProperty(state: any, data: FormData){
    ...
    const validator = new Validator(body, property ? __UpdatePropertySchema.rules : __PropertySchema.rules)
    ...
    if(property){
        uploadedImages = (galleryFiles.length > 0) ? await Promise.all(galleryFiles.map(file => upload(file, 'properties/' + random()))) : property.gallery
        featuredImage = image ? await upload(image, 'properties/'+ random() ) : property.image
    }else{
